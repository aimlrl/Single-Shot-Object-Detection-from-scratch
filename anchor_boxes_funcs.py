...
generated_anchor_boxes = generate_anchor_boxes((5,5,4),(300,300,3))


# In[24]:


generated_anchor_boxes[0,3,0,:]
...
def fetch_all_positive_anchor_boxes(iou,num_unique_categories,anchor_boxes,gt_info,is_normalize,
    ...
    if is_normalize == True:
        ...
        offsets_xy = all_positive_anchor_boxes_gt_info[:,0:2] - anchor_boxes[all_positive_anchor_boxes[:,0],0:2]
        offsets_xy = offsets_xy/anchor_boxes[all_positive_anchor_boxes[:,0],2:4]
        offsets_xy = offsets_xy/0.1
        
        offsets_wh = np.log(all_positive_anchor_boxes_gt_info[:,2:4]/anchor_boxes[all_positive_anchor_boxes[:,0],2:4])
        ...
        offsets = all_positive_anchor_boxes_gt_info[:,0:4] - anchor_boxes[all_positive_anchor_boxes[:,0]]
...
def intersection(bboxes1,bboxes2):
    ...
    bboxes1_topmost_left_corner = np.tile(bboxes1_topmost_left_corner,reps=(1,batch1,1))
    
    bboxes2_topmost_left_corner = np.expand_dims(bboxes2[:,[0,2]],axis=0)
    bboxes2_topmost_left_corner = np.tile(bboxes2_topmost_left_corner,reps=(batch2,1,1))
    ...
    bboxes1_bottommost_right_corner = np.tile(bboxes1_bottommost_right_corner,reps=(1,batch1,1))
    
    bboxes2_bottommost_right_corner = np.expand_dims(bboxes2[:,[1,3]],axis=0)
    bboxes2_bottommost_right_corner = np.tile(bboxes2_bottommost_right_corner,reps=(batch2,1,1))
...
def union(bboxes1,bboxes2):
    ...
    bboxes1_area = np.tile(bboxes1_area, reps=(1,batch1))
    ...
    bboxes2_area = np.tile(bboxes2_area, reps=(batch2,1))
